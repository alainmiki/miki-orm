...
from mikiorm import configure, settings
from mikiorm.migrations.engine import MigrationEngine
from mikiorm.migrations.operations import (
...
from mikiorm.models.fields import CharField, IntegerField, TextField, ForeignKey
...
def test_create_table_operation_generates_sql():
    ...
    columns = []
    for fname, fobj in Tmp._meta.fields.items():
        if hasattr(fobj, 'deconstruct'):
            path, attr_name, args, kwargs = fobj.deconstruct()
            columns.append({
                "name": fname,
                "field_type": path,
                "field_kwargs": kwargs
    ...
    op = CreateTable("tmp_table", columns)
...
def test_add_and_drop_field():
    ...
    add = AddField("dummy", IntegerField(null=True))
    sql = add.to_sql("sqlite")
    assert any("ADD COLUMN age INTEGER" in s for s in sql)

    drop = add.reverse()
    assert isinstance(drop, RemoveField)  # DropField alias points to RemoveField
...
def test_create_and_drop_index():
    ...
    # CreateIndex expects dict with columns
    index_def = {"columns": ["code"], "unique": True}
    create = CreateIndex("idxtest", index_def)
...
def test_alter_field_noop():
    ...
    op = AlterField("alter_me", val=new_field)
    sqls = op.to_sql("sqlite")
...
def test_migration_engine_creates_files(tmp_path):
    ...
    from mikiorm import settings as s
