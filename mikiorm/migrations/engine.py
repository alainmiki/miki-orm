...
class MigrationEngine:
    ...
    def _write_operation_call(self, lines: list[str], op: operations.MigrationOperation, 
        ...
        if op_type == "create_table":
            ...
            # short = field_type.rsplit(".", 1)[-1] # Not used
    ...
    async def async_migrate_direct(self, operations: list[operations.MigrationOperation], connection: Any = None) -> None:
        """Asynchronously apply operations inside a transaction."""
        if connection is None:
            from ..settings import async_connection_manager
            connection = await async_connection_manager.get_connection()

        from ..transactions import async_atomic
        try:
            async with async_atomic(connection=connection):
                from ..query.safe_builder import get_safe_builder
                from ..settings import settings as mikiorm_settings
                db_config = mikiorm_settings.get_database("default")
                builder = get_safe_builder(db_config.engine)
                schema_editor = _SchemaEditor(connection, builder)
                
                for op in operations:
                    # Note: _SchemaEditor is currently sync-based for SQL generation
                    schema_editor.execute_operation(op)
        except Exception:
            raise
...
class _SchemaEditor:
    ...
    def execute_operation(self, op: operations.MigrationOperation) -> None: # type: ignore
    ...
    def _create_table(self, op: operations.CreateTable) -> None:
        from .engine import MigrationEngine as ME # type: ignore
    ...
    def _create_index(self, op: operations.CreateIndex) -> None:
        ...
        unique = index.get("unique", False) # type: ignore
    ...
    def _drop_index(self, op: operations.DropIndex) -> None:
        ...
        # quoted_table = self.builder.quote_table(table) # Not used
    ...
    def _delete_table(self, op: operations.DeleteModel) -> None:
        ...
        quoted_table = self.builder.quote_table(table)
        sql = f"DROP TABLE {quoted_table}" # IF EXISTS is handled by the operation
        self.connection.execute(sql, ())
